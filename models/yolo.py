...
class Model(nn.Module):
    def __init__(self, cfg='yolor-csp-c.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        self.model, self.save, self.output_idx = parse_model(deepcopy(self.yaml), ch=[ch])  # model, savelist
        self.names = [str(i) for i in range(self.yaml['nc'])]  # default names
        # print([x.shape for x in self.forward(torch.zeros(1, ch, 64, 64))])

        # Build strides, anchors
        for idx in self.output_idx:
            m = self.model[idx]  # Detect()
            ...
            if isinstance(m, IDetect):
                ...
                m.anchors /= m.stride.view(-1, 1, 1)
                self.stride = m.stride
                self._initialize_biases(idx)  # only run once
                # print('Strides: %s' % m.stride.tolist())
    ...
    def forward_once(self, x, profile=False):
        ...
        if self.training:
            return [output[i] for i in self.output_idx]
        else:
            return [output[self.output_idx[0]][0], (output[self.output_idx[0]][1], output[self.output_idx[1]])]
        
    def _initialize_biases(self, index=-1, cf=None):  # initialize biases into Detect(), cf is class frequency
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    output = []
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head'] + d['segHead']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC, 
            ...
            c2 = ch[f] // 2
        elif m in [Detect, IDetect, IAuxDetect, IBin, IKeypoint, ISegment]:
        ...
        if i == 0:
            ch = []
        ch.append(c2)
    return nn.Sequential(*layers), sorted(save), sorted(output)
...
if __name__ == '__main__':
    ...
    # if opt.profile:
    #     img = torch.rand(1, 3, 640, 640).to(device)
    #     y, y_output = model(img, profile=True)

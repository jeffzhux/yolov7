...
from utils.general import check_requirements, xyxy2xywh, xywh2xyxy, xywhn2xyxy, xyn2xy, segment2box, segments2boxes, \
    resample_segments, clean_str, xyxy2xywhn
...
def create_dataloader(path, imgsz, batch_size, stride, opt, data_dict=None, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=8, image_weights=False, quad=False, prefix=''):
    # Make sure only the first process in DDP process the dataset first, and the following others can use the cache
    with torch_distributed_zero_first(rank):
        dataset = LoadImagesAndLabels(path, imgsz, batch_size,
                                      data_dict = data_dict, # class_id
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, data_dict=None, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        if cache_path.is_file():
            ...
            cache, exists = self.cache_labels_json(cache_path, prefix), False
        ...
        if cache_images:
            ...
            seg_results = ThreadPool(8).imap(lambda x: load_image(*x, is_seg=True), zip(repeat(self), range(n)))
            pbar = tqdm(enumerate(zip(results, seg_results)), total=n)
            for i, x, seg_x in pbar:
                if cache_images == 'disk':
    ...
    def _convert(self, size, box):
        ...
        w, h = w*dw, h*dh
        return (x,y,w,h)
    def cache_labels_json(self, path=Path('./labels.cache'), prefix=''):
        ...
        for i, (im_file, lb_file) in enumerate(pbar):
            try:
                ...
                # segments = cv2.imread('.'.join(im_file.replace('images', 'seg_gt').split('.')[:-1]+['png']), cv2.IMREAD_GRAYSCALE)# BGR #[]
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if random.random() < 0.8:
                img, labels, seg = load_mosaic(self, index)
            else:
            ...
            if random.random() < hyp['mixup']:
                if random.random() < 0.8:
                    img2, labels2, seg2 = load_mosaic(self, random.randint(0, len(self.labels) - 1))
                else:
            ...
            (img, seg), ratio, pad = letterbox((img, seg), shape, auto=False, scaleup=self.augment)
    ...
    def collate_fn(batch):
        ...
        return torch.stack(img, 0), torch.cat(label, 0), path, shapes, torch.stack(masks, 0)

    @staticmethod
    def collate_fn4(batch):
        img, label, path, shapes, masks = zip(*batch)  # transposed
        n = len(shapes) // 4
        img4, label4, path4, shapes4, masks4 = [], [], path[:n], shapes[:n], []
        ...
        for i in range(n):  # zidane torch.zeros(16,3,720,1280)  # BCHW
            ...
            if random.random() < 0.5:
                im = F.interpolate(img[i].unsqueeze(0).float(), scale_factor=2., mode='bilinear', align_corners=False)[
                    0].type(img[i].type())
                ma = F.interpolate(masks[i].unsqueeze(0).float(), scale_factor=2., mode='bilinear', align_corners=False)[
                    0].type(masks[i].type())
                l = label[i]
            else:
                im = torch.cat((torch.cat((img[i], img[i + 1]), 1), torch.cat((img[i + 2], img[i + 3]), 1)), 2)
                ma = torch.cat((torch.cat((masks[i], masks[i + 1]), 1), torch.cat((masks[i + 2], masks[i + 3]), 1)), 2)
        ...
        return torch.stack(img4, 0), torch.cat(label4, 0), path4, shapes4, torch.stack(masks4, 0)
...
class LoadImagesAndLabelsAndMasks(LoadImagesAndLabels):  # for training/testing
    ...
    def __init__(
        ...
        super().__init__(path, img_size, batch_size, data_dict, augment, hyp, rect, image_weights, cache_images, single_cls,
                         stride, pad, prefix)
...
# Ancillary functions --------------------------------------------------------------------------------------------------
def load_image(self, index, is_seg = False):
    # loads 1 image from dataset, returns img, original hw, resized hw
    path = self.img_files[index]
    if is_seg:
        img = self.segs[index]
        path = '.'.join(path.replace('images', 'seg_gt').split('.')[:-1]+['png'])
    else:
        img = self.imgs[index]
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        if i == 0:  # top left
            img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
            seg4 = np.full((s * 2, s * 2, seg.shape[2]), 0, dtype=np.uint8)
            x1a, y1a, x2a, y2a = max(xc - w, 0), max(yc - h, 0), xc, yc  # xmin, ymin, xmax, ymax (large image)
            x1b, y1b, x2b, y2b = w - (x2a - x1a), h - (y2a - y1a), w, h  # xmin, ymin, xmax, ymax (small image)
        elif i == 1:  # top right
            x1a, y1a, x2a, y2a = xc, max(yc - h, 0), min(xc + w, s * 2), yc
    ...
    img4, labels4, seg4 = copy_paste(img4, labels4, seg4, probability=self.hyp['copy_paste'])
    
    img4, labels4, seg4 = random_perspective(img4, labels4, seg4,
                                       degrees=self.hyp['degrees'],
                                       translate=self.hyp['translate'],
                                       scale=self.hyp['scale'],
                                       shear=self.hyp['shear'],
...
def load_mosaic9(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        if i == 0:  # center
            ...
            seg9 = np.full((s * 3, s * 3, seg.shape[2]), 0, dtype=np.uint8)  # base image with 4 tiles
            h0, w0 = h, w
        ...
        seg9[y1:y2, x1:x2] = seg[y1 - pady:, x1 - padx:]  # img9[ymin:ymax, xmin:xmax]
        hp, wp = h, w  # height, width previous

    # Offset
    yc, xc = [int(random.uniform(0, s)) for _ in self.mosaic_border]  # mosaic center x, y
    img9 = img9[yc:yc + 2 * s, xc:xc + 2 * s]
    seg9 = seg9[yc:yc + 2 * s, xc:xc + 2 * s]
    ...
    img9, labels9, seg9 = copy_paste(img9, labels9, seg9, probability=self.hyp['copy_paste'])
    img9, labels9, seg9 = random_perspective(img9, labels9, seg9,
                                       degrees=self.hyp['degrees'],
                                       translate=self.hyp['translate'],
                                       scale=self.hyp['scale'],
                                       shear=self.hyp['shear'],
...
def random_perspective(img, targets=(), segments=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0,
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if perspective:
            ...
            segments = cv2.warpPerspective(segments, M, dsize=(width, height), borderValue=(0, 0, 0))
        else:  # affine

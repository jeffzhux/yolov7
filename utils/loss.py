...
class ComputeSegLoss:
    # Compute losses
    def __init__(self, model, cfg, autobalance=False):
        super(ComputeSegLoss, self).__init__()
        ...
        det = model.module.model[self.yaml['output_idx'][0]] if is_parallel(model) else model.model[self.yaml['output_idx'][0]]  # Detect() module
    ...
    def __call__(self, preds, targets, masks):  # predictions, targets, model
        ...
        return loss * bs, torch.cat((lseg, lbox, lobj, lcls, loss)).detach()
...
class ComputeSegLossOTA:
    # Compute losses
    def __init__(self, model, cfg, autobalance=False):
        super(ComputeSegLossOTA, self).__init__()
        device = next(model.parameters()).device  # get model device
        h = model.hyp  # hyperparameters
